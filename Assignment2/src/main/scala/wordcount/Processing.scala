...
class Processing {
  ...
  def countWords(l: List[String]): List[(String, Int)] = {
    ...
    l.foldLeft( //links falten
      ...
      (count, word) =>
        count + // Map fuellen und die Count Zahl erhohen
          (word -> (count.getOrElse(word, 0) + 1))
    } // get word +0 wenn Wort nicht vorhanden ansonsten +1 danach +1 da Wort ja einmal min. da
